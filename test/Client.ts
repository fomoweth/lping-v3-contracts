...
initTestEnv({ title: "Client", tracer: { enabled: false } })
    .set({
        ...
        action: async (env) => {
            ...
            for (const pool of POOLS) {
                for (const duration of DURATIONS) {
                    ...
                    const [weth, token, zeroForOne] = !!isWrappedNative(
                        token0.address
                    )
                        ? [token0, token1, false]
                        : [token1, token0, true];

                    const { traders, clients } = await loadFixture(
                        completeFixture
                    );
                    ...
                    const trader = traders.find(
                        (trader) =>
                            !!isSameAddress(trader.address, ownerAddress)
                    )!;
                    ...
                    const { sqrtPriceX96: sqrtRatioX96 } = await getPoolState(
                        poolContract
                    );
                    ...
                    const payloads: string[] = [
                        encodePullTokens(token.address, tokenAmountDesired),
                        encodeMint(
                            assumption,
                            duration,
                            token0.address,
        ...
        action: async (env) => {
            ...
            for (const pool of POOLS) {
                for (const duration of DURATIONS) {
                    ...
                    const payloads: string[] = [
                        encodePullTokens(token.address, amountIn),
                        encodeSwap(
                            [
                                {
                                    adapterId: await v3Swap.id(),
        ...
        action: async (env) => {
            ...
            for (const pool of POOLS) {
                for (const duration of DURATIONS) {
                    ...
                    const { collateralAmount, borrowAmount } =
                        await getCollateralAndBorrowAmounts(
                            assumption,
                            collateralToken,
                            borrowToken,
                            ethAmount,
        ...
        action: async (env) => {
            ...
            for (const pool of POOLS) {
                for (const duration of DURATIONS) {
                    ...
                    const payloads: string[] = [
                        encodePullTokens(token.address, tokenAmount),
                        encodeSwap(
                            [
                                {
                                    adapterId: await v3Swap.id(),
        ...
        action: async (env) => {
            ...
            for (const pool of POOLS) {
                for (const duration of DURATIONS) {
                    ...
                    const payloads: string[] = [
                        encodePullTokens(weth.address, ethAmount),
                        encodeSupplyAndBorrow(
                            await eulerAdapter.id(),
                            collateralToken.address,
                            borrowToken.address,
